...
mf_IHC <- function(Folder_name = "", Path, File_type = "csv", 
                         ...
                         Result_folder = "Results", Plot_folder = "Plots", Region_folder = "Region", All_folder = "All",
                         DV_folder = "DV", Individual_folder = "Individual_data", Hist_folder = "Histogram",
  ...
  luminancehistdata <- function(rawdata){
    ...
    tmp <- read.csv(rawdata, skip = 10, fileEncoding = "Shift-JIS") %>% 
      dplyr::select(5, 7) %>% 
      rename("Luminance Max" = "輝度..最大.") %>% 
      rename("Luminance Mean" = "輝度..平均.") %>% 
