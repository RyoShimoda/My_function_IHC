...
このリポジトリには、Immunohistochemistryのデータ分析とグラフの描画を効率化するためのR自作関数が格納されています。コードを実行すると、自動的にグラフが描画・保存され、解析用のデータセット (csv) が出力されます。Immunohistochemistryにおける切片画像の写真には、 (BZ-X710, Keyence) を用い、解析にはBZ-X analyzer software (Keyence) を用いることを想定しています。
